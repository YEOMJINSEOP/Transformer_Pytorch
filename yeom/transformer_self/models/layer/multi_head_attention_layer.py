...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def __init__(self, d_model, h, qkv_fc, out_fc, dr_rate=0):
        ...
        self.h = h
        self.q_fc = copy.deepcopy(qkv_fc)  # (d_embed, d_model(= h x d_k))
        self.k_fc = copy.deepcopy(qkv_fc)  # (d_embed, d_model(= h x d_k))
        self.v_fc = copy.deepcopy(qkv_fc)  # (d_embed, d_model(= h x d_k))
    ...
    def calculate_attention(self, query, key, value, mask):
        ...
        def transform(x, fc):  # (n_batch, seq_len, d_embed)
            ...
            out = out.view(n_batch, -1, self.h, self.d_model //
                           self.h)  # (n_batch, seq_len, h, d_k)
        ...
        out = out.contiguous().view(n_batch, -1, self.d_model)  # (n_batch, h, d_model)
